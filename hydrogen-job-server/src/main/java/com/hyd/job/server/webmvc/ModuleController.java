...
import static jodd.exception.ExceptionUtil.exceptionStackTraceToString;
...
public class ModuleController {
  ...
  public ModelAndView moduleAction(
    ...
    if (moduleOperation != null) {
      try {
        ...
        log.error("执行操作出现异常, user={}, module={}, operation={}",
          (user.getUserId() + "/" + user.getUserName()), modulePath, operation, e);
        return error("执行出现异常: " + e + "\n" + exceptionStackTraceToString(e));
      }
    } else {
      return error("没有找到该功能");
    }
  }

  private ModelAndView error(String message) {
    return new ModelAndView("/admin/error").addObject("message", message);
  }
